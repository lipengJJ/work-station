...
<template>
  ...
  <div v-else class="space-y-4">
    ...
    <div class="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background-deep))] p-4">
      <h3 class="mb-2 text-xs font-bold text-[hsl(var(--muted-foreground))]">13F 机构持仓</h3>
      ...
      <div v-else-if="institutions.holdings.length === 0" class="py-8 text-center text-xs text-[hsl(var(--muted-foreground))]">
    ...
    <div class="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background-deep))] p-4">
      <div class="mb-2 flex items-center justify-between">
        <h3 class="text-xs font-bold text-[hsl(var(--muted-foreground))]">Form 4 内部人交易（最近 {{ insiders?.transactions.length ?? 0 }} 份披露，共 {{ insiders?.total_form4_filings ?? 0 }} 份历史 Form 4）</h3>
        <span class="text-[10px] text-[hsl(var(--muted-foreground))]">公开市场主动买入单独高亮，行权/税务处置/股权激励不代表看多看空信号</span>
      </div>
      <div v-if="flatTransactions.length === 0" class="py-8 text-center text-xs text-[hsl(var(--muted-foreground))]">{{ NO_DATA_TEXT }}</div>
      <Table v-else :columns="insiderColumns" :data-source="flatTransactions" :pagination="{ pageSize: 15 }" row-key="index_url" size="small">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'owner_name'">
            <div class="font-semibold text-[hsl(var(--foreground))]">{{ record.owner_name }}</div>
            <div class="text-[10px] text-[hsl(var(--muted-foreground))]">{{ record.officer_title }}</div>
          </template>
