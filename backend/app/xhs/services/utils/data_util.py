...
# 素材下载并发度：图集笔记内部并发下几张图/视频封面+正文并发下载用的线程池大小。
# 打的是 xhscdn.com 静态资源域名，风控敏感度和签名 API（搜索/笔记详情）不是一回事，
# 浏览器本身加载页面图片对同一域名也会并发好几个连接，这里做并发不影响上层"整个采集
# 任务串行跑"的防风控设计（那个针对的是需要签名 header 的接口调用）。
_MEDIA_DOWNLOAD_WORKERS = 5

# 素材下载复用一个 Session + 连接池，而不是每次 requests.get() 都新建连接——同一个任务
# 里往往是几百次对同一个 CDN 域名的请求，复用连接省掉重复的 TCP/TLS 握手开销。
# Session 在多线程下并发调用 .get() 是安全的（连接池自己管理），这里没有对 session 做
# 任何按请求变化的可变状态修改。
_media_session = requests.Session()
_media_adapter = HTTPAdapter(pool_connections=10, pool_maxsize=_MEDIA_DOWNLOAD_WORKERS * 2)
...
def _media_download_headers():
    # xhscdn 会拒绝没有浏览器 UA / Referer 的裸请求（常表现为 405），复用站点通用请求头
...
def download_media(path, name, url, type):
    ...
    if file_path.exists() and file_path.stat().st_size > 0:
        # 已经下载过（任务重试、进程重启后 requeue 等场景），跳过，不重新占用一次请求
        return
    headers = _media_download_headers()
    if type == 'image':
        response = _media_session.get(url, headers=headers, timeout=REQUEST_TIMEOUT)
        response.raise_for_status()
        content = response.content
        with open(file_path, mode="wb") as f:
            f.write(content)
    elif type == 'video':
        res = _media_session.get(url, headers=headers, stream=True, timeout=REQUEST_TIMEOUT)
...
def download_note(note_info, path, save_choice):
    ...
    if note_type == '图集' and save_choice in ['media', 'media-image', 'all']:
        ...
        if image_list:
            # 图集内多张图片并发下载；任何一张失败都让异常冒泡出去，外层 @retry 仍然是
            # 对整个 download_note 重试（这个粒度暂时不改，已经记在 TODO 里）
            with ThreadPoolExecutor(max_workers=min(_MEDIA_DOWNLOAD_WORKERS, len(image_list))) as executor:
                futures = [
                    executor.submit(download_media, save_path, f'image_{img_index}', img_url, 'image')
        ...
        # 封面图和视频正文是两个独立文件，并发下载而不是先后来
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = []
            if video_cover:
                futures.append(executor.submit(download_media, save_path, 'cover', video_cover, 'image'))
            if video_addr:
                futures.append(executor.submit(download_media, save_path, 'video', video_addr, 'video'))
            for future in futures:
                future.result()
    return save_path
